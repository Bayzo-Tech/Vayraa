  ...
  return (
    <div className="min-h-screen bg-white flex flex-col pb-10">
      ...
      <div className="sticky top-0 z-10 bg-white/90 backdrop-blur-md px-4 py-3 flex items-center gap-3 border-b border-gray-100">
        ...
        <div>
          <h1 className="text-lg font-bold text-black">Track Order</h1>
          <p className="text-xs text-gray-500">#{orderId.slice(0, 8).toUpperCase()}</p>
        </div>
      </div>
      ...
      <div className="p-4 space-y-4">
        ...
        <div className="bg-white border border-gray-100 shadow-sm rounded-2xl p-5">
          <h2 className="font-bold text-black mb-4">Order Status</h2>
                ...
                return (
                  <div key={step.key} className="flex gap-4">
                    <div className="flex flex-col items-center">
                      <div className={`w-8 h-8 rounded-full flex items-center justify-center border-2 transition-all flex-shrink-0 ${isActive ? 'border-primary bg-primary/10 scale-110' : isDone ? 'border-green-500 bg-green-50' : 'border-gray-200 bg-white'}`}>
                        {isDone && !isActive ? (
                      ...
                      {!isLast && (
                        <div className={`w-0.5 h-8 mt-1 rounded-full ${idx < currentStep ? 'bg-green-500' : 'bg-gray-200'}`} />
                      )}
                    </div>
                    <div className={`pb-6 flex-1 ${isLast ? 'pb-0' : ''}`}>
                      <p className={`font-semibold text-sm ${isActive ? 'text-primary' : isDone ? 'text-black' : 'text-gray-400'}`}>
                        {step.label}
                      </p>
                      <p className={`text-xs mt-0.5 ${isActive ? 'text-gray-500' : isDone ? 'text-gray-500' : 'text-gray-350'}`}>
                        {step.desc}
                      </p>
                    </div>
                  </div>
        ...
        {isOutForDelivery && !isDelivered && !otpSuccess && (
          <div className="bg-white border-2 border-primary/40 rounded-2xl p-5 shadow-sm">
            <div className="flex items-center gap-2 mb-3">
              <span className="text-xl">🔐</span>
              <div>
                <p className="font-bold text-black text-sm">Enter Delivery OTP</p>
                <p className="text-gray-500 text-xs">Your delivery partner will share the OTP</p>
              </div>
            </div>
            <input
              type="text"
              inputMode="numeric"
              className="w-full bg-white border border-gray-200 focus:border-primary rounded-xl px-4 py-3 text-black text-center text-2xl tracking-[0.5em] font-mono outline-none transition-colors mb-3"
              placeholder="— — — — — —"
              value={otp}
        ...
        {otpSuccess && (
          <div className="bg-green-50 border border-green-200 rounded-2xl p-5 text-center">
            ...
            <p className="text-green-600 font-bold">Order Delivered!</p>
            <p className="text-gray-500 text-xs mt-1">Thank you for ordering with Vayra!</p>
          </div>
        )}
        ...
        {isDelivered && (
          <div className="bg-green-50 border border-green-200 rounded-2xl p-5 text-center">
            ...
            <p className="text-gray-500 text-xs mt-1">Enjoy your beach food!</p>
          </div>
        )}
        ...
        {isDelivered && !order.reviewed && order.vendorId && foodIds.length > 0 && (
          <button
            ...
            className="w-full bg-white border-2 border-primary text-primary font-bold py-3.5 rounded-2xl active:scale-95 transition-transform"
          >
            ⭐ Write a Review
          </button>
        )}
        ...
        {order.deliveryPartnerName && (
          <div className="bg-white border border-gray-100 shadow-sm rounded-2xl p-4">
            <p className="text-xs text-gray-500 uppercase tracking-wider mb-2">🛵 Delivery Partner</p>
            <p className="text-black font-bold">{order.deliveryPartnerName}</p>
        ...
        <div className="bg-white border border-gray-100 shadow-sm rounded-2xl p-5">
          ...
          <div className="space-y-2 mb-4">
            {order.items?.map((item: { name: string; quantity: number; price: number }, idx: number) => (
              <div key={idx} className="flex justify-between items-center py-1.5 border-b border-gray-100 last:border-0">
                ...
                <span className="text-black text-sm font-semibold">₹{item.price * item.quantity}</span>
              </div>
            ))}
          </div>
          <div className="space-y-2 border-t border-gray-100 pt-3">
            <div className="flex justify-between text-sm">
              ...
              <span className="text-black">₹{order.itemTotal || order.totalAmount}</span>
            </div>
            {(order.deliveryFee ?? 0) > 0 && (
              <div className="flex justify-between text-sm">
                <span className="text-gray-500">Delivery Fee</span>
                <span className="text-black">₹{order.deliveryFee}</span>
              </div>
            )}
            {(order.packingFee ?? 0) > 0 && (
              <div className="flex justify-between text-sm">
                <span className="text-gray-500">Packing Fee</span>
                <span className="text-black">₹{order.packingFee}</span>
              </div>
            )}
        ...
        {order.location && (
          <div className="bg-white border border-gray-100 shadow-sm rounded-2xl p-4 flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-black font-semibold text-sm">{order.location.area} — {order.location.zone}</p>
            </div>
          </div>
        )}
        ...
        {order.razorpayPaymentId && (
          <div className="bg-white border border-gray-100 shadow-sm rounded-2xl p-4">
            <p className="text-xs text-gray-500 uppercase tracking-wider mb-2">Payment ID</p>
        ...
        {!isDelivered && !isCancelled && (
          <div className="bg-white border border-gray-100 shadow-sm rounded-2xl p-4 flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-black font-bold">20–30 mins</p>
            </div>
          </div>
        )}
